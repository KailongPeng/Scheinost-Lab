function [q_s, r_pearson, r_rank, y, mask] = mCPM(all_mats, all_behav, thresh, k, seed)
    ...
    %   [r_pearson, r_rank, y, mask] = mCPM(all_mats, all_behav, 0.1, 10, 213)
    %
    ...
    for i_fold = 1 : k
        ...
        train_mats_weighted = zeros(num_edge, num_train);
        ...
        for j_train = 1:num_train
            train_mats_temp = sum((squeeze(train_mats(:, j_train, :)) - mean_feature) .* weight, 2);
        ...
        for j_test = 1 : num_test
             test_mats_temp = sum((squeeze(test_mats(:, j_test, :)) - mean_feature) .* weight, 2);
        ...
        mask(:, :, i_fold) = triu(squareform(edge_idx));
