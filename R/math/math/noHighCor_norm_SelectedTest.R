...
noHighCor_norm_SelectedTest <- read.table("raw_data/noHighCor_norm_SelectedTest_test_no_norm_no_nan_header.txt", header=TRUE, sep=",")

noHighCor_norm_SelectedTest <- data.frame(sapply(noHighCor_norm_SelectedTest, as.numeric))

write.csv(noHighCor_norm_SelectedTest, file = "output/noHighCor_norm_SelectedTest.csv", row.names = FALSE)

library(lavaan)
noHighCor_norm_SelectedTest <- read.table("output/noHighCor_norm_SelectedTest.csv", header=TRUE, sep=",")

# check for number of factors
psych::scree(noHighCor_norm_SelectedTest)
psych::fa.parallel(noHighCor_norm_SelectedTest)

# examine exploratory factor analysis (EFA) for noHighCor_norm_SelectedTest
sequence <- seq(20)
for (i in sequence)
{
  i
  fac1 <- factanal(noHighCor_norm_SelectedTest, i, rotation = "promax")
  print(fac1, cutoff = .30)
  fl <- round(unclass(fac1$loadings), 3)
  fl
  file <- paste("output/noHighCor_norm_SelectedTest-",toString(i),"factor-efa-factor-loadings.csv", collapse = "")
  write.csv(fl, file)
}
...
fits$m7 <- lavaan::cfa(models$m7, data = noHighCor_norm_SelectedTest)
  ...
  #generate coefficiant and estimate Factor scores
  b <- coef(fits$m3)
  ...
  for (i in sequence)
  {
    ...
    file <- paste("output/noHighCor_norm_SelectedTest_",toString(i),"FactorScores.csv",sep = "")
