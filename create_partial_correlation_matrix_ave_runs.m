...
if ~isdir([folder '/partial_correlation_by_session'])
    mkdir([folder '/partial_correlation_by_session']);
end
subjectList = [1:12];
sessionList = [1:4];
...
for curr_subject = 1:size(subjectList,2)
    ...
    for curr_session = 1:size(sessionList,2)
        ...
        savefile = [folder '/partial_correlation_by_session/' subject_session{1}(1:11)];
        ...
        tic
        rho = partialcorr(curr_session_mean);
        toc
        save(savefile,'rho');
        rho = [];
        filename = [];
